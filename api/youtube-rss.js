...
function parseRSS(xml) {
  ...
  while ((m = re.exec(xml)) !== null) {
    ...
    desc  = desc.replace(/<[^>]*>/g,'').replace(/&amp;/g,'&').slice(0, 300).trim();
    pub   = pub.split('T')[0];
...
module.exports = async function(req, res) {
  ...
  try {
    ...
    if (result.status !== 200) {
      return res.status(500).json({ error: 'YouTube 응답 오류: ' + result.status });
    }
    var videos = parseRSS(result.body);
    if (!videos.length) {
      return res.status(500).json({ error: '영상 파싱 실패', raw: result.body.slice(0, 300) });
    }
